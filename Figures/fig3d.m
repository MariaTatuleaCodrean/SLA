...
for kk=1:length(mf)
    ...
    outputfile = ['../Simulations/Data/output_ABS_n_10000_Nsim_10_sigV_per_muV_' mf_str '.mat'];
...
% Define variables
length_count = zeros(length(mf),length(tplot_main),Nsim);
agent_count  = zeros(length(mf),length(tplot_main),Nsim);
avg_len      = zeros(length(mf),length(tplot_main),Nsim);
...
avg_vel = nan(length(mf),nmax_inset,length(tplot_inset),Nsim);
...
% PLOT MAIN FIGURE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fig = figure('Units', 'centimeters','Position', [15,10,8.3,5.5]);
...
title(leg,'$\sigma_V / \overline{V}$','Interpreter','Latex')
