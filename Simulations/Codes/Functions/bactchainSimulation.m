function [x0,t0,tend,tsim,vel,len,way,active,alpha] = bactchainSimulation(n0,X0,vel_pdf,outputfile,Nsim,Nevent,Pright)
...
%   vel_pdf     distrib         bacterial speed distrib (experiments)
%
% Optional arguments
...
%   len     double array        chain length as number of bacteria
...
len    = nan(Nsim,n0+Nevent);  % chain length as number of bacteria
...
for jj = 1:Nsim
    ...
    len(jj,1:n0) = 1;            % Initialize lengths
...
save(outputfile,'n0','X0','vel_pdf','Nsim','Nevent','x0','t0','tend','tsim','vel','len','way','active','alpha','-append')
end